...
(defvar *fsm-pgm
  (list
   ...
   (@def-pgm :input-course-check
             (@fsm-data)
             (@clt "FSM :input-course-check" data)
   ...
   (@def-pgm :computer
             (@fsm-data)
             (@clt "FSM :computer" data)
             (computer (car data))
             (values))
   (@def-pgm :comp-calc
             (@fsm-data)
             (@clt "FSM :comp-calc" data)
             (display ": ~a~%" data)
             (setq *x0* (car data))
             (setq *y0* (cadr data))
             (display "FINAL COORDINATES X Y?")
   ...
   (@def-pgm :comp-calc-final-co
             (@fsm-data)
             (display ": ~a~%" data)
             (@clt "FSM :comp-calc-final-co" data)
             (setq *x1* (car data))
             (setq *y1* (cadr data))
             (disp-direct-dist *x0* *y0* *x1* *y1*)
             (state :mloop-command)
